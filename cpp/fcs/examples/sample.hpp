...
#pragma warning(disable : 4251)
  ...
  // Number of entries in ZoopEnum
  enum { ZOOPENUM_NUMBER_ENTRIES = 3 };

  //! Returns char array correpsonding to value of ZoopEnum
  extern  char const* to_c_str(ZoopEnum e);

  //! Assigns to value the entry associated with text in char array or "INVALID_ZOOPENUM" if not valid
  extern  void from_c_str(char const* txt, ZoopEnum &value);

  inline std::ostream& operator<<(std::ostream &out, ZoopEnum e) {
    return out << to_c_str(e);
  } 
  ...
  public:
    ...
    //! Returns char array correpsonding to value of ZoopEnum
    friend char const* to_c_str(ZoopEnum e);

    //! Assigns to value the entry associated with text in char array or "INVALID_ZOOPENUM" if not valid
    friend void from_c_str(char const* txt, ZoopEnum &value);

    friend inline std::ostream& operator<<(std::ostream &out, sample::ZoopEnum e) {
      return out << to_c_str(e);
    }
    ...
    // Number of entries in VooEnum
    enum { VOOENUM_NUMBER_ENTRIES = 3 };

    friend inline void from_c_str(char const* txt, VooEnum &value) {
      if(0 == std::strcmp("va_xyz", txt)) {
        value = va_xyz;
        return;
      }
      if(0 == std::strcmp("vb_xyz", txt)) {
        value = vb_xyz;
        return;
      }
      if(0 == std::strcmp("vc", txt)) {
        value = vc;
        return;
      }
      std::string msg("No VooEnum value corresponding to: ");
      msg += txt;
      throw std::runtime_error(txt);
    }

    friend inline char const* to_c_str(sample::VooEnum e) {
      char const* values[] = {
        "va_xyz",
        "vb_xyz",
        "vc"
      ...
      size_t index(e);
      return ((index < VOOENUM_NUMBER_ENTRIES)? values[index] : "INVALID_VOOENUM");
    }

    friend inline std::ostream& operator<<(std::ostream &out, sample::VooEnum e) {
      return out << to_c_str(e);
    }
